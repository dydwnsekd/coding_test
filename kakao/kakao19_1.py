...
# Enter, leave, change 분류  
# uid : nickname로 저장하는거 
# uid : nickname 기준으로 변경하는 해서 출력하는 것 

def split_mode(input_list):
    uid_list = list()
...
def uid_nick(input_list):
    uid_dict = dict()

    for i in input_list:
        uid_dict[i.split(' ')[1]] = i.split(' ')[2]
...
if __name__ == "__main__":
    ...
    uid_list = split_mode(input_list)
    uid_dict = uid_nick(uid_list)

    for i in input_list:
        mode = i.split(' ')[0]
        nick = uid_dict[i.split(' ')[1]]
        if mode == 'Enter':
            answer.append('%s님이 들어왔습니다.' % nick)
        elif mode == 'Leave':
            answer.append('%s님이 나갔습니다.' % nick)
